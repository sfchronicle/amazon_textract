...
with open('json/COH/COH1_499.json') as f:
  data_1 = json.load(f)

with open('json/COH/COH500_.json') as f:
  data_2 = json.load(f)
...
school_building = districts[district]['school_name']
incident_date = districts[district]['incident_date']
restraint_start = districts[district]['restraint_began']
restraint_end = districts[district]['restraint_ended']
seclusion_start = districts[district]['seclusion_began']
seclusion_end = districts[district]['seclusion_ended']
...
for incident in data:
  ...
  for line in incident:
    ...
    if any(time in line for time in restraint_start):
      if has_number(line):
        incident_dict['restraint_began']=line
      else:
        incident_dict['restraint_began'] = get_next_line(incident,line)

    if any(time in line for time in restraint_end):
      if has_number(line):
        incident_dict['restraint_end']=line
      else:
        incident_dict['restraint_end'] = get_next_line(incident,line)

    if any(time in line for time in seclusion_start):
      if has_number(line):
        incident_dict['seclusion_start']=line
      else:
        incident_dict['seclusion_start'] = get_next_line(incident,line)

    if any(time in line for time in seclusion_end):
      if has_number(line):
        incident_dict['seclusion_end']=line
      else:
        incident_dict['seclusion_end'] = get_next_line(incident,line)

    try:
      if any(type in line for type in types):
          incident_dict['restraint_type'] = get_next_line(incident,line)
    except:
      pass

    if any(des in line for des in description):
      incident_dict['description']  = get_next_line(incident,line)
